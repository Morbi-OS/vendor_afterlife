...
func init() {
	android.RegisterModuleType("afterlife_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = afterlifeExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = afterlifeExpandVariables(ctx, dep_file)
	...
	cmd := afterlifeExpandVariables(ctx, String(g.properties.Cmd))
