...
def is_afterlife_gerrit(remote_url):
    p = urllib.parse.urlparse(remote_url)
    return p.hostname == "review.afterlifeos.org"
...
def main():
    parser = argparse.ArgumentParser(
        ...
        patches from AfterlifeOS's Gerrit instance (or any gerrit instance of your choosing)
    ...
    parser.add_argument(
        "-g",
        "--gerrit",
        default="https://review.afterlifeos.org",
        metavar="",
...
def do_git_fetch_pull(args, item):
    ...
    # Try fetching from GitHub first if using afterlife gerrit
    if is_afterlife_gerrit(args.gerrit):
        if args.verbose:
    ...
    # If not using the afterlife gerrit or github failed, fetch from gerrit.
    if args.verbose:
        if is_afterlife_gerrit(args.gerrit):
            print(
