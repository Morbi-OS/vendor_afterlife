...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/AfterlifeOS-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.afterlife.device | grep -q "$AFTERLIFE_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $AFTERLIFE_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function afterliferemote()
{
    ...
    then
        ...
        local PFX="AfterlifeOS/"
    else
        local PROJECT=$REMOTE
    fi

    local AFTERLIFE_USER=$(git config --get review.review.afterlifeos.org.username)
    if [ -z "$AFTERLIFE_USER" ]
    then
        git remote add afterlife ssh://review.afterlifeos.org:29418/$PFX$PROJECT
    else
        git remote add afterlife ssh://$AFTERLIFE_USER@review.afterlifeos.org:29418/$PFX$PROJECT
    fi
...
function afterlifegerrit() {
    ...
    local user=`git config --get review.review.afterlifeos.org.username`
...
function afterliferebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "AfterlifeOS Gerrit Rebase Usage: "
    ...
    git fetch "http://review.afterlifeos.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.afterlife.device | grep -q "$AFTERLIFE_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
